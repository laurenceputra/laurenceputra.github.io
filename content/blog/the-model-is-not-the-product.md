...
I built a travel research workflow in OpenClaw, an open‑source AI harness running on a VPS. It orchestrates multiple models, routes tasks, integrates web search, delegates to sub‑agents, maintains memory, loads skills. My starting setup was simple: Mimo v2 Pro and Gemini search. A one‑paragraph prompt.
...
The prompt that works in one harness is not automatically good in another. Instructions are not portable the way people assume. Their effectiveness depends on the harness they operate inside. A large part of the value was not the model. It was the harness helping to produce the right task specification in the first place.Over time, I have found myself gradually shifting toward Qwen 3.6 and Mimo v2 (both variants) to bring costs down. The harness I have designed has narrowed the gap between my OpenClaw setup and the Claude and ChatGPT apps for general writing and deep research. The models are not all equal. But inside a well‑built system, the difference is smaller than most people expect.
...
The harness thesis stops being theoretical the moment real money is at stake.

I am also running portfolio management research through these same systems. Real capital at risk sharpens the feedback loop. You learn faster when the output is not just interesting, but consequential. A research workflow that looks impressive in a demo is one thing. A research workflow that helps you make better decisions under uncertainty is something else.

The question I am currently investigating: why does my ChatGPT research desk seem to help construct portfolios with strong Sortino profiles, and what exactly is still missing in my OpenClaw setup?

My current guess is that the answer will not be "the model is better." It will be something more structural in the harness. Better research flow. Better context carryover. Better synthesis. Better ranking of uncertainty. Better decision support at the point where ambiguity is still high.

That is the kind of gap that matters. Not benchmark scores. Not token throughput. Whether the system helps you act well when the answer is not yet clear.
...
This article was produced with inputs from Qwen 3.6 Plus, Mimo v2 Flash, Mimo v2 Pro, GPT-5.4-mini, GPT-5.4, and Sonnet 4.6, all running through an open-source harness. Which, if you have been paying attention, is the point.